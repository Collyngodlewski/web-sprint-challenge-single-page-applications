...
1. In 1-2 sentences, explain what React's `useRouteMatch` hook is used for. useRouteMatch hook is used to attempt to match the current url. Its used to get access to the matched data without rendering.
1. How would you explain form validation to someone who has never programmed before?  Validation is for error messaged if a form has not been correctly filled out
1. In 1-2 sentences, define end to end testing. End to end testing tests from beginning to end of an application. It helps replicate real scenarios.
...
- [] A homepage that has a "/" route and links to your form (button, nav bar, or any other type of link is acceptable but must have an id of "order-pizza") done
- [] A order form that has a "/pizza" route and shows the form done
- [] A form with an id of "pizza-form" done
- [] A name text input field with an id of "name-input" done
- [] Validation for name and the error message is "name must be at least 2 characters" (Use this exact error message to make sure tests pass) ::: VERY IMPORTANT TO USE THAT EXACT ERROR MESSAGE (casing included!) done
- [] A dropdown for pizza size with an id of "size-dropdown" done
- [ ] A checklist for toppings - at least 4 (hint: name each separately!)
- [] Text input for special instructions with an id of "special-text" done
- [] An Add to Order button that has an id of "order-button" and that submits form and returns a database record of name, size, toppings and special instructions kinda done? keeps updating every time i type
